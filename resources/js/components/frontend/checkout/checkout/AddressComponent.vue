<template>
    ...
    <div v-if="show" :id="'checkout-section-' + slug"
        ...
        <div v-if="logged" class="grid grid-cols-1 sm:grid-cols-2 gap-6 p-4">
        ...
        <div v-else class="p-4 pt-0">
             <form class="w-full pt-4" @submit.prevent="save">
                <div class="form-row">
                    ...
                    <div class="form-col-12 sm:form-col-6">
                        ...
                        <div :class="errors.phone ? 'invalid' : ''" class="field-control flex items-center border border-[#D9DBE9] rounded-lg shadow-sm">
                            <div class="w-fit flex-shrink-0 dropdown-group ml-3">
                                <button type="button" class="flex items-center gap-1 dropdown-btn">
                                    {{ address.flag }}
                                    <span class="whitespace-nowrap flex-shrink-0 text-xs font-semibold">
                                        {{ address.form.country_code }}
                                    </span>
                                </button>
                                <ul
                                    class="p-1.5 w-24 rounded-lg shadow-xl absolute top-8 left-0 z-10 border border-gray-200 bg-white scale-y-0 origin-top dropdown-list !h-52 !overflow-x-hidden !overflow-y-auto thin-scrolling">
                            ...
                            <input v-model="address.form.phone" v-on:keypress="phoneNumber($event)"
                                ...
                                class="pl-2 text-sm w-full h-full h-12 outline-none" />
                        </div>
                    </div>
                    ...
                    <div class="form-col-12 sm:form-col-6">
                        ...
                        <vue-select
                            class="w-full h-12 px-4 rounded-lg text-base capitalize border border-[#D9DBE9] hover:border-primary/30 focus-within:border-primary/30 transition-all duration-500 appearance-none shadow-sm"
                            id="country" v-model="address.form.country"
                    ...
                    <div class="form-col-12 sm:form-col-6">
                        ...
                        <vue-select
                            class="w-full h-12 px-4 rounded-lg text-base capitalize border border-[#D9DBE9] hover:border-primary/30 focus-within:border-primary/30 transition-all duration-500 appearance-none shadow-sm"
                            id="state" v-model="address.form.state"
                    ...
                    <div class="form-col-12 sm:form-col-6">
                        ...
                        <vue-select
                            class="w-full h-12 px-4 rounded-lg text-base capitalize border border-[#D9DBE9] hover:border-primary/30 focus-within:border-primary/30 transition-all duration-500 appearance-none shadow-sm"
                            id="city" v-model="address.form.city"
...
export default {
    ...
    methods: {
        ...
        callCities: function (stateName) {
            ...
            this.$store.dispatch('frontendCountryStateCity/citiesByState', {
                state: stateName,
                country: this.address.form.country
            })
                .then((res) => {
        ...
        guestForm: function() {
            // For guest, always keep the method updated with the form data
            this.method(this.address.form);
        },
        reset: function () {
        ...
        save: function () {
            if (!this.logged) {
                // For guest, we don't save to DB, we just pass the data up
                this.method(this.address.form);
                return;
            }
            try {
            ...
            if (Object.keys(this.selectedAddress).length > 0) {
                ...
                this.$store.dispatch("frontendAddress/edit", address.id).then(async (res) => {
                    ...
                    if (address.state !== "") {
                        ...
                        await this.$store.dispatch('frontendCountryStateCity/citiesByState', {
                            state: address.state,

<template>
    ...
    <!-- Mobile Footer with Newsletter -->
    <footer class="pt-8 pb-20 bg-secondary sm:hidden">
        <div class="container">
            <div class="flex flex-col items-center">
                <router-link :to="{ name: 'frontend.home' }" class="mb-6">
                    <img class="w-32" :src="setting.theme_footer_logo" alt="logo">
                </router-link>

                <form @submit.prevent="saveSubscription" class="w-full max-w-sm mb-8 px-4">
                    <label class="block mb-4 text-center font-medium text-white">
                        {{ $t('message.subscribe_to_our_newsletter') }}
                    </label>
                    <div class="flex w-full h-11 rounded-full p-1 bg-white">
                        <input type="email" v-model="subscriptionProps.post.email"
                            :placeholder="$t('label.your_email_address')" class="w-full h-full pl-4 pr-2 text-sm text-heading outline-none">
                        <button type="submit"
                            class="text-xs font-bold capitalize flex-shrink-0 px-5 h-full rounded-full bg-primary text-white transition-colors hover:bg-heading">
                            {{ $t('button.subscribe') }}
                        </button>
                    </div>
                </form>
                ...
                <nav v-if="setting.social_media_facebook || setting.social_media_twitter || setting.social_media_instagram || setting.social_media_youtube"
                    class="flex flex-wrap items-center justify-center gap-5 mb-8">
                    <a v-if="setting.social_media_facebook" target="_blank" :href="setting.social_media_facebook"
                        class="lab-fill-facebook w-8 h-8 !leading-8 text-center rounded-full text-sm text-secondary bg-white transition-all duration-300 hover:bg-primary hover:text-white"></a>
                    <a v-if="setting.social_media_twitter" target="_blank" :href="setting.social_media_twitter"
                        class="lab-fill-x w-8 h-8 !leading-8 text-center rounded-full text-sm text-secondary bg-white transition-all duration-300 hover:bg-primary hover:text-white"></a>
                    <a v-if="setting.social_media_instagram" target="_blank" :href="setting.social_media_instagram"
                        class="lab-fill-instagram w-8 h-8 !leading-8 text-center rounded-full text-sm text-secondary bg-white transition-all duration-300 hover:bg-primary hover:text-white"></a>
                    <a v-if="setting.social_media_youtube" target="_blank" :href="setting.social_media_youtube"
                        class="lab-fill-youtube w-8 h-8 !leading-8 text-center rounded-full text-sm text-secondary bg-white transition-all duration-300 hover:bg-primary hover:text-white"></a>
                </nav>

                <div class="w-full grid grid-cols-2 gap-8 mb-8 px-4 text-center">
                    <div>
                    ...
                    <div>
                        <h4 class="text-lg font-semibold capitalize mb-4 text-white border-b border-white/10 pb-2 inline-block px-2">{{ $t('label.legal') }}</h4>
                        <nav v-if="legalPages.length > 0" class="flex flex-col gap-3">
                            <router-link v-for="legalPage in legalPages"
                                class="text-sm font-medium capitalize text-gray-300 transition-colors hover:text-primary"
                                :to="{ name: 'frontend.page', params: { slug: legalPage.slug } }">
                                {{ legalPage.title }}
                ...
                <div class="text-center px-4 pt-6 border-t border-white/5 w-full">
                    <p class="text-[10px] font-medium text-gray-400 tracking-wider uppercase mb-2">{{ setting.site_copyright }}</p>
                    <div class="flex justify-center gap-3 mt-4">
                        <a target="_blank" v-if="setting.site_android_app_link" :href="setting.site_android_app_link">
                            <img class="h-7" :src="setting.image_play_store" alt="app">
                        </a>
                        <a target="_blank" v-if="setting.site_ios_app_link" :href="setting.site_ios_app_link">
                            <img class="h-7" :src="setting.image_app_store" alt="app">
                        </a>
                    </div>
                </div>
            </div>

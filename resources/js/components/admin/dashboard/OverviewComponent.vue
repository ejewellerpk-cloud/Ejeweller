<template>
    ...
    <div class="mb-9">
        ...
        <div class="row">
            ...
            <div class="col-12 sm:col-6 xl:col-3">
                <div class="bg-[#00b074] p-4 rounded-lg flex items-center gap-4">
                    <div class="w-12 h-12 rounded-full flex items-center justify-center bg-white">
                        <i class="lab-fill-users text-[#00b074] text-2xl lab-font-size-24"></i>
                    </div>
                    <div>
                        <h3 class="font-medium tracking-wide capitalize text-white">Active Admins</h3>
                        <h4 class="font-semibold text-[22px] leading-[34px] text-white">{{ active_admins }}</h4>
                    </div>
                </div>
            </div>
            <div class="col-12 sm:col-6 xl:col-3">
                <div class="bg-[#ff9f43] p-4 rounded-lg flex items-center gap-4">
                    <div class="w-12 h-12 rounded-full flex items-center justify-center bg-white">
                        <i class="lab-fill-users text-[#ff9f43] text-2xl lab-font-size-24"></i>
                    </div>
                    <div>
                        <h3 class="font-medium tracking-wide capitalize text-white">Active Customers</h3>
                        <h4 class="font-semibold text-[22px] leading-[34px] text-white">{{ active_customers }}</h4>
                    </div>
                </div>
            </div>
        </div>
...
export default {
    ...
    methods: {
        ...
        activeUsers: function () {
            this.loading.isActive = true;
            this.$store.dispatch("dashboard/activeUsers").then((res) => {
                this.active_admins = res.data.data.active_admins;
                this.active_customers = res.data.data.active_customers;
                this.loading.isActive = false;
            }).catch((err) => {
                this.loading.isActive = false;
            });

                        ...
                        <tr class="db-table-body-tr" v-for="product in products" :key="product">
                            <td class="db-table-body-td">
                                <div class="flex items-center gap-2">
                                    <span>{{ textShortener(product.name, 40) }}</span>
                                    <span v-if="product.videos && product.videos.length > 0" 
                                          class="flex items-center justify-center w-5 h-5 rounded-full bg-red-100 text-red-600 shadow-sm shrink-0"
                                          :title="product.videos.length + ' video(s) uploaded'">
                                        <svg class="w-2.5 h-2.5 fill-current ml-0.5" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">

            ...
            <div class="flex flex-wrap gap-y-5 items-end justify-between">
                <div>
                    <div class="flex flex-wrap items-start gap-y-2 gap-x-6 mb-5">
                        ...
                        <div class="flex items-center gap-2 mt-1.5">
                            <span :class="'text-sm capitalize h-5 leading-5 px-2 rounded-3xl text-[#FB4E4E] bg-[#FFDADA]' + statusClass(order.payment_status)">
                                {{ enums.paymentStatusEnumArray[order.payment_status] }}
                            </span>
                            <span :class="'text-xs capitalize h-5 leading-5 px-2 rounded-3xl ' + orderStatusClass(order.status)">
                    ...
                    <ul class="flex flex-col gap-2">
                        ...
                        <li class="text-xs">
                            ...
                            <span class="text-heading">{{ order.payment_method_name }}</span>
                        </li>
                        <li class="text-xs">
                            {{ $t('label.order_type') }}:
                            <span class="text-heading">{{ $t('menu.whatsapp_order') }}</span>
                        </li>
                        <li v-if="order.note" class="text-xs">
                            {{ $t('label.note') }}:
                            <span class="text-heading">{{ order.note }}</span>
                        </li>
                    </ul>
                </div>
                ...
                <div class="flex flex-wrap gap-3" v-else-if="order.status !== enums.orderStatusEnum.REJECTED && order.status !== enums.orderStatusEnum.CANCELED">
                    <div class="relative">
                        ...
                        <i class="lab lab-line-chevron-down lab-font-size-16 absolute top-1/2 right-3.5 -translate-y-1/2 text-primary"></i>
                    </div>
                    <div class="relative">
            ...
            <div class="db-card-body">
                <div class="pl-3">
                    <div class="mb-3 pb-3 border-b last:mb-0 last:pb-0 last:border-b-0 border-gray-2"
                        v-for="product in orderProducts" :key="product.id">
                        <div class="flex items-center gap-3 relative">
                            ...
                            <div class="flex-auto overflow-hidden">
                                <h4 class="text-sm capitalize whitespace-nowrap overflow-hidden text-ellipsis">{{ product.product_name }}</h4>
                                <p class="text-sm overflow-hidden">{{ product.variation_names }}</p>
                                <span class="text-sm font-semibold">{{ product.subtotal_currency_price }}</span>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div class="db-card p-1">
                    <ul class="flex flex-col gap-2 p-3 border-b border-dashed border-[#EFF0F6]">
                        ...
                        <li class="flex items-center justify-between text-heading">
                            ...
                            <span class="text-sm leading-6 capitalize font-semibold text-[#1AB759]">{{ order.shipping_charge_currency_price }}</span>
                        </li>
                    </ul>
                    <div class="flex items-center justify-between p-3">
                        <h4 class="text-sm leading-6 font-bold capitalize">{{ $t('label.total') }}</h4>
                        <h5 class="text-sm leading-6 font-bold capitalize">{{ order.total_currency_price }}</h5>
                    </div>
                </div>
            </div>

            <div class="col-12" v-if="orderAddress.length > 0" v-for="address in orderAddress" :key="address.id">
                <div class="db-card">
                    <div class="db-card-header">
                        <h3 class="db-card-title" v-if="address.address_type === enums.addressTypeEnum.SHIPPING">{{ $t('label.shipping_address') }}</h3>
                        <h3 class="db-card-title" v-else-if="address.address_type === enums.addressTypeEnum.BILLING">{{ $t('label.billing_address') }}</h3>
                    </div>
                    <div class="db-card-body">
                        <h4 class="font-semibold text-sm capitalize text-[#374151] mb-3">{{ address.full_name }}</h4>
                        <ul class="flex flex-col gap-3 py-4 border-t border-[#EFF0F6]">
                            ...
                            <li v-if="address.phone" class="flex items-center gap-2.5">
                ...
                <div class="db-card">
                    ...
                    <div class="db-card-body">
                        <div class="flex items-center gap-3 mb-4">
                            ...
                            <h4 class="font-semibold text-sm capitalize text-[#374151]">{{ textShortener(orderUser.name, 20) }}</h4>
                        </div>
                        <ul class="flex flex-col gap-3 py-4 border-t border-[#EFF0F6]">
                            <li class="flex items-center gap-2.5">
                                <i class="lab lab-line-mail lab-font-size-14"></i>
                            ...
                            <li class="flex items-center gap-2.5">
                                <i class="lab lab-line-call-calling lab-font-size-14"></i>
                                <span class="text-xs" dir="ltr">{{ orderUser.country_code }}{{ orderUser.phone }}</span>
                            </li>
                        </ul>
                    </div>
                </div>
...
export default {
    name: "WhatsappOrderShowComponent",
    components: {
        LoadingComponent,
    },
    data() {
        return {
            ...
            enums: {
                ...
                orderStatusObject: [
                    { name: this.$t("label.confirmed"), value: orderStatusEnum.CONFIRMED },
                    { name: this.$t("label.on_the_way"), value: orderStatusEnum.ON_THE_WAY },
    ...
    mounted() {
        ...
        this.$store.dispatch('whatsappOrder/show', this.$route.params.id).then(res => {
    ...
    computed: {
        order: function () {
            return this.$store.getters['whatsappOrder/show'];
        },
        orderProducts: function () {
            return this.$store.getters['whatsappOrder/orderProducts'];
        },
        orderUser: function () {
            return this.$store.getters['whatsappOrder/orderUser'];
        },
        orderAddress: function () {
            return this.$store.getters['whatsappOrder/orderAddress'];
        },
    },
    methods: {
        ...
        orderStatus: function (e) {
            this.loading.isActive = true;
            this.$store.dispatch("whatsappOrder/changeStatus", {
        ...
        changePaymentStatus: function (e) {
            this.loading.isActive = true;
            this.$store.dispatch("whatsappOrder/changePaymentStatus", {
                id: this.$route.params.id,

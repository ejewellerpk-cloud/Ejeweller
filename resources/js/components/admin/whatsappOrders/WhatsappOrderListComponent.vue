            ...
            <div class="table-filter-div" id="whatsapporderlist-filter">
                <form class="p-4 sm:p-5 mb-5 w-full d-block" @submit.prevent="search">
                    <div class="row">
                        ...
                        <div class="col-12 sm:col-6 md:col-4 xl:col-3">
                            <label for="user_id" class="db-field-title">{{ $t("label.customer") }}</label>
                ...
                <table class="db-table stripe" id="print">
                    <thead class="db-table-head">
                        <tr class="db-table-head-tr">
                            ...
                            <th class="db-table-head-th hidden-print" v-if="permissionChecker('whatsapp-orders')">{{ $t('label.action') }}</th>
                        </tr>
                    </thead>
                    <tbody class="db-table-body" v-if="orders.length > 0">
                        <tr class="db-table-body-tr" v-for="order in orders" :key="order.id">
                            ...
                            <td class="db-table-body-td">{{ textShortener(order.user.name, 20) }}</td>
                            ...
                            <td class="db-table-body-td hidden-print" v-if="permissionChecker('whatsapp-orders')">
                                <div class="flex justify-start items-center gap-1.5">
                                    <SmIconViewComponent :link="'admin.whatsapp.orders.show'" :id="order.id" v-if="permissionChecker('whatsapp-orders')" />
                                    <SmIconDeleteComponent @click="destroy(order.id)" v-if="permissionChecker('whatsapp-orders')" />
                                </div>
                            </td>
                        </tr>
                    </tbody>
...
export default {
    name: "WhatsappOrderListComponent",
    components: {
        TableLimitComponent,
        PaginationSMBox,
        PaginationBox,
    ...
    data() {
        return {
            ...
            printObj: {
                id: "print",
                popTitle: this.$t("menu.whatsapp_orders"),
            },
            props: {
                search: {
                    paginate: 1,
    ...
    computed: {
        orders: function () {
            return this.$store.getters['whatsappOrder/lists'];
        },
        customers: function () {
            return this.$store.getters['user/lists'];
        },
        pagination: function () {
            return this.$store.getters['whatsappOrder/pagination'];
        },
        paginationPage: function () {
            return this.$store.getters['whatsappOrder/page'];
        }
    },
    methods: {
        permissionChecker(e) {
        ...
        clear: function () {
            ...
            this.props.search.source = sourceEnum.WHATSAPP;
        ...
        list: function (page = 1) {
            ...
            this.$store.dispatch('whatsappOrder/lists', this.props.search).then(() => {
        ...
        destroy: function (id) {
            appService.destroyConfirmation().then(() => {
                this.loading.isActive = true;
                this.$store.dispatch('whatsappOrder/destroy', { id: id, search: this.props.search }).then(() => {
                    this.loading.isActive = false;
                    alertService.successFlip(null, this.$t('menu.whatsapp_orders'));
                }).catch((err) => {
                    this.loading.isActive = false;
        ...
        xls: function () {
            this.loading.isActive = true;
            this.$store.dispatch("whatsappOrder/export", this.props.search).then((res) => {
                this.loading.isActive = false;
                ...
                link.download = this.$t("menu.whatsapp_orders");
                link.click();

...
export default {
    ...
    currencyFormat(amount, decimal, currency, position) {
        let dec = (decimal === null || decimal === undefined || decimal === '') ? 2 : parseInt(decimal);
        if (isNaN(dec)) dec = 2;
        if (parseInt(position) === currencyPositionEnum.LEFT) {
            return (currency || '') + parseFloat(amount || 0).toFixed(dec);
        } else {
            return parseFloat(amount || 0).toFixed(dec) + (currency || '');
        }
    },
    ...
    decimalPoint: function (num, length = 2) {
        let dec = (length === null || length === undefined || length === '') ? 2 : parseInt(length);
        if (isNaN(dec)) dec = 2;
        return Number.parseFloat(num || 0).toFixed(dec);
    },

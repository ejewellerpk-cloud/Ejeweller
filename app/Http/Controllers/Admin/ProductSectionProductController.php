...
class ProductSectionProductController extends AdminController implements HasMiddleware
{
    ...
    {
        return [
            new Middleware('permission:product-sections_show', only: ['index']),
            new Middleware('permission:product-sections_show', only: ['store']),
            new Middleware('permission:product-sections_show', only: ['destroy']),
        ];
    }

...
class ProductImport implements ToModel, WithHeadingRow, WithValidation, SkipsOnFailure, SkipsEmptyRows
{
    ...
    public function model(array $row)
    {
        $product = Product::create([
            ...
            'show_stock_out'             => isset($row['show_stock_out']) && trim($row['show_stock_out']) !== '' ? AppEnum::ask($row['show_stock_out']) : \App\Enums\Ask::NO,
            'refundable'                 => isset($row['refundable']) && trim($row['refundable']) !== '' ? AppEnum::ask($row['refundable']) : \App\Enums\Ask::NO,
            'maximum_purchase_quantity'  => isset($row['maximum_purchase_quantity']) && trim($row['maximum_purchase_quantity']) !== '' ? $row['maximum_purchase_quantity'] : 1,
            'low_stock_quantity_warning' => isset($row['low_stock_quantity_warning']) && trim($row['low_stock_quantity_warning']) !== '' ? $row['low_stock_quantity_warning'] : 5,
        ]);
    ...
    public function rules(): array
    {
        return [
            ...
            'show_stock_out'             => ['nullable', 'string'],
            'refundable'                 => ['nullable', 'string'],
            'maximum_purchase_quantity'  => ['nullable', 'numeric', 'max_digits:10'],
            'low_stock_quantity_warning' => ['nullable', 'numeric', 'max_digits:10'],

    ...
    public function store(ProductVideoRequest $request, Product $product)
    {
        try {
            DB::transaction(function () use ($request, $product) {
                ...
                if ($request->video_provider == 20 && !$request->link) {
                    $data['link'] = 'uploading';
                }
                $this->productVideo = ProductVideo::create($data + ['product_id' => $product->id]);
                if ($request->video_provider == 20 && $request->hasFile('file')) {
                    $this->productVideo->addMediaFromRequest('file')->toMediaCollection('product_video');
        ...
        try {
            DB::transaction(function () use ($request, $product, $productVideo) {
                if ($productVideo->product_id == $product->id) {
                    $data = $request->validated();
                    if ($request->video_provider == 20 && !$request->link) {
                        $data['link'] = $productVideo->link; // Keep old link if no new link/file
                    }
                    $productVideo->update($data);
                    if ($request->video_provider == 20 && $request->hasFile('file')) {
                        $productVideo->clearMediaCollection('product_video');
                        $productVideo->addMediaFromRequest('file')->toMediaCollection('product_video');
